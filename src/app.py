...
def predict(img):
    ...
    # net.load_state_dict(torch.load('./src/dog_cat.pt', map_location=torch.device('cpu'))) # local
    net.load_state_dict(torch.load('./dog_cat.pt', map_location=torch.device('cpu'))) # render
    #　データの前処理
    img = transform(img)
